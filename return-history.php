<?php include("inc/header.php"); ?>  
		...
		<div class="container">
			...
			<div class="from_border_3"> 
					<h4 class="text-center"> <span style="color:green; font-weight:bold;">Sales Return History 
					...
					<table id="example" class="display table table-striped table-reponsive table-bordered" style="width:90%;">
						...
						<tbody>
							<?php 
								$query="SELECT * from return_invoices where reciept_cat=1 order by id DESC"; 
								...
								?>
							<tr>
								<td class="text-center">INV-<?php echo $rs['return_invoice_id']; ?></td>
								...
								<td class="text-center"><a href="return_print_invoice.php?inv='<?php echo $rs['return_invoice_id']?>'" target="blank" class="btn btn-success btn-sm"> <i class="fas fa-print"></i> </a> </td>
						...
						<tfoot class="bg-info">
							<?php 
								$query="SELECT SUM(amount_due) as due, SUM(amount_paid) as paid from return_invoices where reciept_cat=1 order by id DESC"; 
								$results=$db->select($query);
								$id=0;
								if($results){
									while($rs=$results->fetch_assoc()){
